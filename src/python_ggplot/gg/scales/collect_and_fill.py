            ...
            if not discrete_kind.label_seq:
                if isinstance(scale, (LinearDataScale, TransformedDataScale)):
                    ...
                    if scale.data.reversed:
                        label_seq_opt = list(pd.Series(data.unique()).sort_values(ascending=False))  # type: ignore
                    else:
                        label_seq_opt = list(pd.Series(data.unique()).sort_values())  # type: ignore
                else:
                    label_seq_opt = list(pd.Series(data.unique()).sort_values())  # type: ignore
            else:
...
def add_facets(filled_scales: FilledScales, plot: GgPlot):
    ...
    for fc in facet.columns:
        ...
        fill_scale_data = FillScaleData(scale=scale_, df=None)
